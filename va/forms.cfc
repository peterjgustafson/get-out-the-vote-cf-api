    ...
    <!--- handle GET request (httpmethod),
     ...
     <cffunction name="abhandler"
		...
		<cflock type="exclusive" scope="server" timeout="#createTimeSpan(0,0,0,30)#">
			...
			<cfpdfform source="#getDirectoryFromPath(getCurrentTemplatePath())#ABVirginia.pdf" destination="#getDirectoryFromPath(getCurrentTemplatePath())#temp\#fileName#" action="populate" overwriteData="true">
			...
			<!---VA only requires birth year and uses a single characher blocked fields for input --->
				<cfif isNumeric(url.cust_dobyear) and Len(url.cust_dobyear) EQ 4>
					<cfpdfformparam name="cust_dob_year1" value="#left(url.cust_dobyear,1)#">
					<cfpdfformparam name="cust_dob_year2" value="#mid(url.cust_dobyear,2,1)#">
					<cfpdfformparam name="cust_dob_year3" value="#mid(url.cust_dobyear,3,1)#">
					<cfpdfformparam name="cust_dob_year4" value="#right(url.cust_dobyear,1)#">
				</cfif>
		...
		<cfinclude template="../includes/thankYou/va.cfm" >
